 cat /u01/app/oraglobd/oracle_db_backup.sh
adddate() {
...
sh /u01/app/oraglobd/globsnap | adddate >> /u01/app/oraglobd/oracle-db-backup.log &



cat /u01/app/oraglobd/globsnap
# ==========================================================================================================
...
retention_prod=$(date -d "-7 days" +%Y-%m-%d)
retention_dev=$(date -d "-2 days" +%Y-%m-%d)
...
snaplist=$(aws ec2 describe-snapshots  --query "Snapshots[?(StartTime<='$retention_dev')].[SnapshotId]"  --filters "Name=tag:Database,Values=Oracle" --output text| wc -l)
echo -e "- Snapshots to deleted: [$snaplist]"

if [ `aws ec2 describe-snapshots  --query "Snapshots[?(StartTime<='$retention_dev')].[SnapshotId]"  --filters "Name=tag:Database,Values=Oracle" --output text| wc -l` == 0 ]; then
echo "- [0] Snapshots to be deleted before $retention_dev, Skipping!!"
else
echo "- Initiating Snap-delete"
snap_delete=$(aws ec2 describe-snapshots  --query "Snapshots[?(StartTime<='$retention_dev')].[SnapshotId]"  --filters "Name=tag:Database,Values=Oracle" --output text)
for snapshot in $snap_delete; do
aws ec2 delete-snapshot --snapshot-id $snapshot
done
echo -e "- Snapshot cleanup completed."
fi
...
aws ec2 create-snapshots --instance-specification InstanceId=$insid --tag-specifications 'ResourceType=snapshot,Tags=[{Key="Database",Value="Oracle"},{Key="Deleteon",Value='${futuredt01}'}]' --copy-tags-from-source volume --description "Oracle DB - Snapshot Backups" |  grep SnapshotId | awk '{print $2}' | cut -d "," -f1 | sed -e 's/^"//' -e 's/"$//' | xargs aws ec2 wait snapshot-completed --snapshot-ids
done
